...
export default class JWT {
    ...
    async signToken(headers, payload, signingKey) {
        const signature = await Crypto.HmacSHA256(`${headers}.${payload}`, signingKey);
        return this.makeURLSafe(Crypto.toBase64(new Uint8Array(signature)));
    }

    async toJwt() {
        var date = this.formatDate(this.iat);
        const signingKey = await Crypto.HmacSHA256(date, Crypto.fromBase64(this.base64Secret));
        const encodedHeaders = this.convertObjectToBase64(this.sortObject(this.headers()));
        const encodedPayload = this.convertObjectToBase64(this.sortObject(this.payload()));
        const signature = await this.signToken(encodedHeaders, encodedPayload, signingKey);
